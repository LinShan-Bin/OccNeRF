...
class Metric_FScore():
    ...
    def add_batch(self,semantics_pred,semantics_gt,mask_lidar,mask_camera ):
        ...
        self.tot_acc_dist += acc_dist
        self.tot_cmpl_dist += cmpl_dist
        self.tot_cd += acc_dist + cmpl_dist

    def count_fscore(self):
        base_color, attrs = 'red', ['bold', 'dark']
        acc = self.tot_acc / self.cnt
        cmpl = self.tot_cmpl / self.cnt
        f1_mean = self.tot_f1_mean / self.cnt
        acc_dist = self.tot_acc_dist / self.cnt
        cmpl_dist = self.tot_cmpl_dist / self.cnt
        cd = self.tot_cd / self.cnt
        print(pcolor('\n######## F score: {} #######'.format(f1_mean), base_color, attrs=attrs))
        print(pcolor('######## CD: {} #######'.format(cd), base_color, attrs=attrs))
        return acc, cmpl, f1_mean, acc_dist, cmpl_dist, cd, self.cnt
